...
%     Pfrf = Pfrf.P_carriage_left;
    ...
    standstill = xref > max(xref)-1e-5;
    idx = find(standstill);
    begin = idx(1);
    last = idx(end);
    de = de./factor_standstill;
    de(2*begin:2*last)  = de(2*begin:2*last).*factor_standstill^2;
%% 
    ...
    weight.We_sq = We_sq;
    weight.Wf_sq = Wf_sq;
    weight.Wdf_sq = Wdf_sq;
    weight.Wry_sq = Wry_sq;
    weight.Wdry_sq = Wdry_sq;
...
%             num(k+1) = (-1)^k * nchoosek(i,k);                                  % derivative basis function, i.e., (1-z^-1)/Ts . Feel free to play with the basis functions.
%         end
%             Psi_y_phi(i) = minreal(tf(num,1,Ts,'Variable','z^-1'));
%     end
    ...
    if n_friction_x > 0
        v = brfus_v003(Psi(1).',xref,t,Ts);
        v(abs(v)<1e-6) = 0;
        Psi_friction_x = sign(v);
    else
        Psi_friction_x = [];
    end
    ...
    Phi = zeros(2*Nref,na+nb_x+nb_phi+n_friction_x+n_friction_phi);

        Psi_y_r = zeros(2*Nref,na);
        Psi_y_r(1:2:end) = brfus_v003(Psi(1:na).',xref,t,Ts);
        Psi_y_r_sep = brfus_v003(Psi(1:na).',xref,t,Ts);

        Psi_ff_r = zeros(2*Nref,nb_x + nb_phi+ n_friction_x + n_friction_phi);
        Psi_ff_r(1:2:end,1:nb_x+n_friction_x) = [brfus_v003(Psi(na+1:na+nb_x).',xref,t,Ts),Psi_friction_x];
        Psi_ff_r(2:2:end,nb_x+n_friction_x+1:end) = [brfus_v003(Psi(na+nb_x+1:end).',xref,t,Ts),Psi_friction_phi];

        S_lifted = liftedMIMOMatrix(S_mimo,Nref);
        SP_lifted = liftedMIMOMatrix(GS_mimo,Nref);
        

        Phi(:,1:na) = -S_lifted*Psi_y_r;
        Phi(:,na+1:end) = SP_lifted*Psi_ff_r;

        
size(Wdry_sq,1),size(Psi_ff_r,2)



    X = [We_sq*Phi;
            -Wry_sq*Psi_y_r_sep, zeros(Nref,size(Psi_ff_r,2));
            -Wdry_sq*Psi_y_r_sep, zeros(Nref,size(Psi_ff_r,2));
            zeros(2*Nref,na), -Wf_sq*Psi_ff_r;
            zeros(2*Nref,na), -Wdf_sq*Psi_ff_r];
...
history.theta = zeros(size(Psi,2)+n_friction_x+n_friction_phi,N_trial);
...
history.n_friction_x = n_friction_x;
history.n_friction_phi = n_friction_phi;

%cost
history.J = Nan(N_trial,1);
...
%          theta_delta = FeedforwardUpdate_BFIS_simo_Chiron(na,nb_x,nb_phi,Psi,Nref,S_mimo,GS_mimo,weight,e_j,squeeze(history.r_y(jj,:,:)),f_j,xref,t,Ts);
        [theta_delta, Y] = FeedforwardUpdate_BFIS_simo_lifted(X,Nref,weight,e_j,squeeze(history.r_y(jj,:,:)),f_j);
        ...
        Cff_phi = minreal(Psi_ff_phi*history.theta(na+nb_x+n_friction_x+1:end-n_friction_phi,jj));
        

        f_jplus1 = zeros(Nref,ni);
        if n_friction_x ~= 0
            f_jplus1(:,2) = brfus_v003(Cff_x,xref,t,Ts) + Psi_friction_x*history.theta(na+nb_x+n_friction_x,jj);
        else
            f_jplus1(:,2) = brfus_v003(Cff_x,xref,t,Ts);
        end

        if n_friction_phi ~= 0
            f_jplus1(:,3) = brfus_v003(Cff_phi,xref,t,Ts)+ Psi_friction_phi*history.theta(end,jj);
        else
...
history.Cy = Cy;
history.Cff_x = Cff_x;
history.Cff_phi = Cff_phi;

tf_xx = minreal(Cff_x/Cy);
tf_xphi = minreal(Cff_phi/Cy);
...
bode(tf_xphi);
grid on;
legend; title("Fitting vs $P^{-1}$",'Interpreter','latex');
%% Cost
figure; plot(history.J); title("Costs"); xlabel('Trial');grid on;
...
%voeg gewenste parameters toe voor de visualisatie in de log file 
...
Log.SetGeneralParameters(...
    ...
    "wdf_x",wdf_x,...
    "Wdf_phi",wdf_phi,...
    "x friction", n_friction_x, ...
    "phi friction",n_friction_phi,...
    "Standstill factor",factor_standstill...
...
history.eNorm
Log.SetTrialParameters( ...
    "Trial", 1, ...
    "eNorm_x", history.eNorm(1,2), ...
    "eNorm_phi", history.eNorm(1,3),...
    "Improvement factor x",  0, ...
    "Improvement factor phi", 0 ...
    );

Log.SetTrialParameters( ...
    "Trial", jj-1, ...
    "eNorm_x", history.eNorm(jj-1,2), ...
    "eNorm_phi", history.eNorm(jj-1,3), ...
    "Improvement factor x",  history.eNorm(1,2)/history.eNorm(jj-1,2), ...
    "Improvement factor phi",  history.eNorm(1,3)/history.eNorm(jj-1,3) ...
    );
